...
class _TransferScreenState extends State<TransferScreen> {
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      // Initialize AuthApiService with the base URL from environment variables
      final authApiService =
          AuthApiService(baseUrl: dotenv.env['API_BASE_URL'] ?? '');

      // Get a valid token using the centralized token service
      final token = await authApiService.getValidToken();

      // Fetch user profile using the ApiClient
  ...
  Future<void> _createTransaction(
    ...
    try {
      ...
      final transactionResponse = await http
          .post(
      ...
      if (transactionResponse.statusCode != 200 &&
        ...
        throw Exception(
            'Failed to create transaction: ${transactionResponse.body}');
      }
      ...
      if (transactionId == null) {
        throw Exception('Transaction ID is null in response.');
      }

      // Step 2: Create activity record
      final activityResponse = await http
          .post(
            Uri.parse('$apiBaseUrl/items/activity'),
            headers: {
              'Authorization': 'Bearer $token',
              'Content-Type': 'application/json',
            },
      ...
      if (activityResponse.statusCode != 200 &&
          activityResponse.statusCode != 201) {
        throw Exception('Failed to log activity: ${activityResponse.body}');
      }
    } catch (e, stackTrace) {
      debugPrint('Transaction creation failed: $e');
      debugPrintStack(stackTrace: stackTrace);
      rethrow; // Let caller handle it, or show error UI
    }
  }
...
class _ConfirmTransferScreenState extends State<ConfirmTransferScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Color(0xFF89C7F0),
        elevation: 0,
        leading: BackButton(color: Colors.black87),
        centerTitle: true,
        title: Text('Transfer', style: textStyleVersion2()),
                          ...
                          child: Row(
                            children: [
                              CircleAvatar(
                                ...
                                backgroundColor: Color(0xFF4AAAE6),
                              ),
                              SizedBox(width: 12),
                              Expanded(
                                child: Column(
